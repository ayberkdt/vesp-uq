...
def main():
    ...
    print("Loading 5-day LUNAR test scenarios...")
    scenarios = pd.read_csv("data/test_5days/scenarios.csv")
    metrics = pd.read_csv("data/test_5days/metrics/gpu_batch_per_scenario_metrics.csv")
    
    st_lrps_metrics = metrics[metrics['model'] == 'GPU_ST_LRPS_RK4'].copy()
    ...
    print("Integrating 512 accurate point-mass trajectories for 5 DAYS...")
    # 5 days = 432000 s
    duration_tu = (5 * 86400.0) / TU_s
    t_eval = np.linspace(0, duration_tu, 300) # evaluate 300 points along orbit
