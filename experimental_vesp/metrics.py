...
def radial_cross_radial_error(
    ...
    return {
        "radial_rmse": float(torch.sqrt(torch.mean(radial_error * radial_error)).detach().cpu()),
        "cross_radial_rmse": float(torch.sqrt(torch.mean(cross_error * cross_error)).detach().cpu()),
    }
