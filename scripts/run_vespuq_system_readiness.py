...
This is the one-command answer to "are the measurement gates actually run?". It executes the
pre-result artifacts over the same config set:

* A/B/C gate diagnostics
* exact log-factor attribution with masking validation
* diagonal radial/tangential noise prototype
* source-geometry auto-selection sweep in full mode

The script then writes a compact top-level readiness report and a manifest that also checksums all
subrun artifacts. It does not promote any prototype into the production model.
"""
...
from scripts.run_vespuq_rtn_noise_prototype import run_rtn_noise_prototype  # noqa: E402
from vesp.common.config import load_config  # noqa: E402
from vesp.uq.attribution import run_log_attribution  # noqa: E402
from vesp.uq.gate_diagnostics import run_gate_diagnostics  # noqa: E402
from vesp.uq.geometry_calibration import run_geometry_calibration  # noqa: E402
from vesp.uq.io.run_artifacts import verify_manifest, write_run_artifacts  # noqa: E402
...
def _existing_reports(paths: list[str] | None, *, n_configs: int) -> list[str | None]:
    if paths is None:
        paths = DEFAULT_REPORTS[:n_configs] if n_configs <= len(DEFAULT_REPORTS) else []
    if len(paths) == 0:
    ...
    *,
    ...
    if geometry_ran:
        ...
        for band, verdict in sorted(geometry_verdict.items()):
            bits.append(f"{band}: best={verdict.get('best_geometry')}")
    ...
    rows = [
        ...
        },
        {
            ...
            "detail": f"{attr_cases} cases, masking win-rate {_fmt(mask_win_rate, '.2f')}",
        },
        {
...
def _summary_md(summary: dict[str, Any], rows: list[dict[str, Any]], out_dir: Path) -> str:
    ...
    lines.extend(
        [
            ...
            f"- `{out_dir / 'geometry_calibration' / 'geometry_calibration.md'}` (full readiness, or quick with `--include-geometry`)",
        ]
    )
    return "\n".join(lines) + "\n"
    ...
    *,
    ...
    manifest = write_run_artifacts(
        ...
        text_files={
            ...
            "system_readiness_checks.csv": _csv_text(rows, ["check", "status", "detail"]),
        },
...
def main(argv=None) -> None:
    ...
    reports = _existing_reports(args.reports, n_configs=len(configs))
