...
Status: **planning** (no experiments run yet). This document decomposes the journal-review
feedback into reproducible work packages and maps every requested artifact onto either an
**existing** asset (reuse) or a **new** asset (build). It does not change any manuscript claim;
claims are revised only after Milestone 1 produces reproducible outputs (see §"Claim discipline").
...
| Baseline comparison | Yes (strong) | `scripts/compare_risk_baselines.py` (673 L), `src/vesp/uq/baselines.py`, `outputs/baselines_L60/L90` | Yes | Single seed; no aggregate tables |
| Partial corr / within-bin Spearman | **Partial (already!)** | `compare_risk_baselines.py` → `partial_pearson_given_min_radius`, `within_altitude_bin_spearman` | Yes | No matched-pair test; not aggregated over seeds |
| Rerun-fraction sweep | **Partial (already!)** | `compare_risk_baselines.py` → `altitude_incremental_value.fraction_sweep`, `altitude_incremental_sweep.csv` | Yes | No plots; not multi-seed; few fractions |
...
| Reproducibility manifest + git hash | Yes | `write_run_artifacts(...)`, `run_manifest.json` everywhere | Yes | Add SHA-256 checksums |
| **Multi-seed robustness** | **No** | — | — | Build (WP2) |
| **Rerun-budget curves (plots)** | **No** | — | — | Build (WP3) |
| **Matched-altitude pair test** | **No** | — | — | Build (WP4) |
| **Score-variant ablation (15 variants)** | **No** | `src/vesp/uq/scoring.py` has variants; no ablation harness | — | Build (WP6) |
| **Learned linear/ridge supervisor baseline** | **No** | — | — | Build (WP5) |
| **Raw-vs-calibrated reliability table** | **No** | calibration exists, no before/after | — | Build (WP7) |
| **Unified benchmark suite runner** | **No** | `run_experiment_suite.py` is for source-recovery suites | — | Build (WP1) |
| **Journal report + LaTeX tables/figs** | **No** | — | — | Build (WP12) |
| Makefile target | No (no Makefile) | — | — | Use `pyproject` console-script alias instead |

**Conclusion:** the scientific core is solid; the gap is the *evidence layer* — multi-seed
aggregation, budget curves, a matched-altitude incremental-value test, score ablation, a learned
baseline, and journal-ready packaging.

Relevant `src/vesp/uq/` modules to reuse: `baselines.py`, `scoring.py`, `metrics.py`,
...
### WP1 — Unified benchmark runner skeleton
...
### WP2 — Multi-seed robustness (L60 + L90)
...
- **New:** aggregation helpers in `src/vesp/uq/suite.py` to compute mean±std across seeds for:
...
- **New:** `src/vesp/uq/figures.py` additions (or `suite.py`) to plot capture / lift / error-ratio
...
### WP4 — Altitude-controlled incremental-value diagnostics  ← **highest priority**
...
- **New file:** `scripts/run_altitude_controlled.py` + `src/vesp/uq/altitude_controlled.py`:
...
### WP5 — Expanded baseline set
- **Reuse:** `src/vesp/uq/baselines.py` (`min_altitude_scores`, `low_altitude_exposure_scores`,
  `knn_p95_scores`, `domain_support_scores`, `altitude_residual_expected_scores`).
- **New (in `baselines.py` + suite):**
  ...
  - 5.3 learned linear/ridge/logistic supervisor over features {min_altitude, low_alt_exposure,
    ...
    or top-decile label; strict train/val/test separation. New `src/vesp/uq/learned_supervisor.py`.
...
### WP6 — Score-variant ablation (VESP-UQ score itself)
- **Reuse:** `src/vesp/uq/scoring.py` already exposes scoring modes (expected, supervisor_rel/abs,
  p95, low-alt integral, etc.).
- **New:** `scripts/run_score_ablation.py` evaluating the 15 review variants (expected_error_only,
...
### WP7 — Calibration: raw vs calibrated + reliability
- **Reuse:** `src/vesp/uq/audit.py`, `conformal.py`, `run_calibration_audit.py`.
- **New:** before/after table (raw vs calibrated PICP90, z-std per altitude bin), reliability
...
### WP8 — Source geometry & regularization sensitivity
- **Reuse:** `scripts/geometry_shootout.py`, `regularizer_shootout.py`,
  `outputs/ablation_real_lunar_{shells,regularization}`.
- **New/extend:** grid over n_sources {320,640,1280,2560}, shell radius/depth, #shells, lambda,
...
### WP9 — Trajectory-family diversity
- **Reuse:** `src/vesp/uq/trajectory.py` Keplerian generation; `screening.*` config block.
- **New:** family generators (low-alt near-circular, eccentric perilune, polar, equatorial,
...
### WP10 — Force-risk → trajectory-drift multi-horizon diagnostic
- **Reuse:** `scripts/run_stlrps_propagation.py`, `src/vesp/uq/linear_propagation.py`,
  `benchmark_stm_dispersion.py`.
- **New:** split into (10.1) force-error ranking diagnostic [main claim] and (10.2) drift diagnostic
...
### WP11 — Physical-unit budget screening status
- **Reuse:** `scripts/run_physical_budget_screening.py`, `src/vesp/uq/physical_units.py`.
- **New:** with L60/L90 metadata present (`acceleration_units: km/s^2`), run an absolute-mode budget
...
### WP12 — Journal report, LaTeX tables, figures, tests, reproducibility
- **New report:** `outputs/journal_validation_report.md` with the 14 review sections (exec summary →
